// It is just editing the first ticket. 
...
function CreateTable(){
  ...
  var ele = act_names[0]
  var thing = "Name: " + ele;
  act_names.splice(0,1);
  var txt = document.createTextNode(thing);
  td.appendChild(txt);
  tr1.appendChild(td);
...
function checkForList1(){
  ...
  for(k=0;k<pushingcol1.length;k++){
      ...
      var row = abclist[ind];
      var concat = row+"1";
      biglist.push([concat,pushingcol1[k]])
  ...
  function checkForList2(){
    ...
    for(k=0;k<pushingcol2.length;k++){
      ...
      var row = abclist[ind];
      var concat = row+"2";
      biglist.push([concat,pushingcol2[k]])
  ...
  for(k=0;k<pushingcol3.length;k++){
      ...
      var row = abclist[ind];
      var concat = row+"3";
      biglist.push([concat,pushingcol3[k]])
  ...
  for(k=0;k<pushingcol4.length;k++){
      ...
      var row = abclist[ind];
      var concat = row+"4";
      biglist.push([concat,pushingcol4[k]])
  ...
  for(k=0;k<pushingcol5.length;k++){
      ...
      var row = abclist[ind];
      var concat = row+"5";
      biglist.push([concat,pushingcol5[k]])
  ...
  for(k=0;k<pushingcol6.length;k++){
      ...
      var row = abclist[ind];
      var concat = row+"6";
      biglist.push([concat,pushingcol6[k]])
  ...
  for(k=0;k<pushingcol7.length;k++){
      ...
      var row = abclist[ind];
      var concat = row+"7";
      biglist.push([concat,pushingcol7[k]])
  ...
  for(k=0;k<pushingcol8.length;k++){
      ...
      var row = abclist[ind];
      var concat = row+"8";
      biglist.push([concat,pushingcol8[k]])
      var indexOfRow = abclist.indexOf(row)
      abclist.splice(indexOfRow,1);
      if(row == "a"){
        nums_a += 1
...
function checkBigList(){
  for(var i =0; i<biglist.length; i++){
    ...
    // if(i > 8){
      ...
      // }
      // if(biglist[i][1] >= 79 && biglist[i][1] <=70){
      //   recurse(biglist[i],'undefined8');
      // }
      // if(biglist[i][1] >= 90 && biglist[i][1] <=80){
      //   recurse(biglist[i],'undefined9');
      // }
